...
function Scratchpad:turn_on()
  ...
  if matches[1] and not in_anim then
    ...
    if anim_x then
      anim_x:set(new_x)
      local unsub_x
      unsub_x = anim_x.ended:subscribe(function()
        in_anim = false
        unsub_x()
      end)
    end
    if anim_y then
      anim_y:set(new_y)
      local unsub_y
      unsub_y = anim_y.ended:subscribe(function()
        in_anim = false
        unsub_y()
      end)
    end
...
function Scratchpad:turn_off()
  ...
  if c and not in_anim then
    ...
    if anim_x then
      ...
      local unsub
      unsub = anim_x.ended:subscribe(function()
        in_anim = false
        helpers.client.turn_off(c)
        unsub()
      end)
    ...
    if anim_y then
      ...
      local unsub
      unsub = anim_y.ended:subscribe(function()
        in_anim = false
        helpers.client.turn_off(c)
        unsub()
      end)

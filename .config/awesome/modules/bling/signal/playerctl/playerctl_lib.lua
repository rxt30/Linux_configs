...
function position_cb()
  ...
  if player then
    ...
    if position ~= last_position or length ~= last_length then
      awesome.emit_signal("bling::playerctl::position", position, length, player.player_name)
...
function metadata_cb(player, metadata)
  ...
  if player == manager.players[1] then
    ...
    if player ~= last_player or title ~= last_title or artist ~= last_artist or artUrl ~= last_artUrl then
      awful.spawn.with_line_callback(get_album_art(artUrl), {
        stdout = function(line)
          awesome.emit_signal("bling::playerctl::title_artist_album", title, artist, line, player.player_name)
        end,
      })
...
local function playerctl_enable(args)
  ...
  args.update_on_activity = args.update_on_activity or beautiful.playerctl_update_on_activity

...
local mappings = function()
  ...
  map("n", "<C-m>", ":Telescope find_files <CR>")
  map("n", "<C-n>", ":NvimTreeToggle <CR>")
  map("n", "<C-g>", ":Telescope live_grep <CR>")
  map("n", "<C-p>", ":Telescope projects <CR>")
  map("n", "<F5>", ":let b:caret = winsaveview() <CR>:%SnipRun <CR>:call winrestview(b:caret) <CR>")
  map("n", "<F7>", require'dap'.toggle_breakpoint)
  map("n", "<F8>", require'dap'.continue)
  map("n", "<F9>", require'dap'.step_over)
  map("n", "<F10>", require'dap'.step_into)
  map("n", "<F11>", require("dapui").toggle)
